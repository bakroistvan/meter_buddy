...
| `storage` | Hot counters in `RTC_DATA_ATTR`; LittleFS `/records.bin`, `/sync.dat`, `/stay_awake.dat`, `/brownout.dat` (protection lock + pending `low_battery` / `brownout_lock` flags); getters `available()`, `currentPulses()`, `currentPeriodStart()`, `protectionLocked()`; `MaxUploadRecords` = 128; public `compactRecords()`; `hexdump()` prints file hex to a `Stream` (REPL `h` and diagnostics boot — not the upload path) |
